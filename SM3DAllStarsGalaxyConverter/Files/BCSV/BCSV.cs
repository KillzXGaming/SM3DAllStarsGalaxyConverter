...
namespace SM3DAllStarsGalaxyConverter
{
    ...
    public class BCSV
    {
        ...
        private void Read(BinaryDataReader reader)
        {
            ...
            long strTableSize = reader.BaseStream.Length - strTableOffset;
            Console.WriteLine("strTableSize " + strTableSize);
            ...
            if (strTableSize > 0)
            {
                reader.Seek(strTableOffset, SeekOrigin.Begin);
                StringPool = reader.ReadBytes((int)strTableSize);
            }
        }
                ...
                for (int i = 0; i < fields.Count; i++)
                {
                    ...
                    switch (fields[i].Type)
                    {
                        ...
                        case FieldType.Int16:
                            Values[i] = (ushort)reader.ReadInt16();
                           /* Values[i] = (ushort)((reader.ReadInt16() >> fields[i].Shift) & fields[i].Bitmask);*/
                            break;
                        case FieldType.Byte:
